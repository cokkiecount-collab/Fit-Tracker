...
function ProgramSide({ programmer, userId, db }: Props) {
  ...
  if (valgtProgramId === null) {
    return (
      <>
        <h2>Programmer</h2>
        <input type="text" placeholder="Programnavn" value={programNavn} onChange={(e) => setProgramNavn(e.target.value)} />
        <button onClick={async () => {
          if (!programNavn.trim()) return
          await db.opretProgram(programNavn, userId)
          setProgramNavn("")
        }}>+ Opret program</button>
        <ul>
          {programmer.map((program) => (
            <li key={program.id}>
              {redigerProgram === program.id ? (
                <>
                  <input value={nytProgramNavn} onChange={(e) => setNytProgramNavn(e.target.value)} />
                  <button onClick={async () => {
                    if (!nytProgramNavn.trim()) return
                    await db.opdaterProgram(program.id!, nytProgramNavn)
                    setRedigerProgram(null); setNytProgramNavn("")
                  }}>Gem</button>
                  <button onClick={() => { setRedigerProgram(null); setNytProgramNavn("") }}>Annuller</button>
                </>
              ) : (
                <>
                  <button onClick={() => setValgtProgramId(program.id!)}>{program.navn}</button>
                  <button style={{ marginLeft: "10px" }} onClick={() => { setRedigerProgram(program.id!); setNytProgramNavn(program.navn) }}>✏️</button>
  ...
  if (valgtDagId === null || !valgtProgram) {
    return (
      <>
        ...
        <input type="text" placeholder="Navn på træningsdag" value={dagNavn} onChange={(e) => setDagNavn(e.target.value)} />
        <button onClick={async () => {
          if (!dagNavn.trim()) return
          await db.opretDag(valgtProgramId, dagNavn)
          setDagNavn("")
        }}>+ Opret træningsdag</button>
        <ul>
          {valgtProgram?.dage.map((dag) => (
            <li key={dag.id}>
              {redigerDag === dag.id ? (
                <>
                  <input value={nytDagNavn} onChange={(e) => setNytDagNavn(e.target.value)} />
                  <button onClick={async () => {
                    if (!nytDagNavn.trim()) return
                    await db.opdaterDag(dag.id!, nytDagNavn, valgtProgramId)
                    setRedigerDag(null); setNytDagNavn("")
                  }}>Gem</button>
                  <button onClick={() => { setRedigerDag(null); setNytDagNavn("") }}>Annuller</button>
                </>
              ) : (
                <>
                  <button onClick={() => setValgtDagId(dag.id!)}>{dag.navn}</button>
                  <button style={{ marginLeft: "10px" }} onClick={() => { setRedigerDag(dag.id!); setNytDagNavn(dag.navn) }}>✏️</button>
                  <button style={{ marginLeft: "5px" }} onClick={async () => {
  ...
  return (
    <>
      ...
      <h2>{valgtDag?.navn}</h2>
      <input type="text" placeholder="Øvelsesnavn" value={oevelseNavn} onChange={(e) => setOevelseNavn(e.target.value)} />
      <button onClick={async () => {
        ...
        {valgtDag?.oevelser.map((oevelse) => (
          <li key={oevelse.id}>
            {redigerOevelse === oevelse.id ? (
              <>
                <input value={nytOevelseNavn} onChange={(e) => setNytOevelseNavn(e.target.value)} />
                <button onClick={async () => {
                  if (!nytOevelseNavn.trim()) return
                  await db.opdaterOevelse(oevelse.id!, nytOevelseNavn, valgtDagId, valgtProgramId)
                  setRedigerOevelse(null); setNytOevelseNavn("")
                }}>Gem</button>
                <button onClick={() => { setRedigerOevelse(null); setNytOevelseNavn("") }}>Annuller</button>
              </>
            ) : (
              <>
                <span>{oevelse.navn}</span>
                <button style={{ marginLeft: "10px" }} onClick={() => { setRedigerOevelse(oevelse.id!); setNytOevelseNavn(oevelse.navn) }}>✏️</button>
                <button style={{ marginLeft: "5px" }} onClick={async () => {
